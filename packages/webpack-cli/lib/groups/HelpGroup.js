...
class HelpGroup {
    outputHelp(isCommand = true, subject) {
        ...
        } else if (argsUsed.length > 1 && !argsUsed.includes('--help')) {
            console.error(chalk.red('\nYou provided multiple arguments with help. Please use one argument at a time.\n'));
            process.exit();
    ...
    outputVersion(externalPkg) {
        ...
        if (commandsUsed.length > 1 && invalidArgs.length === 0) {
        ...
        if (invalidArgs.length > 0) {
            console.error(chalk.red(`\nError: Invalid Option '${invalidArgs[0]}'.`));
            console.log(chalk.cyan('Run webpack --help to see available commands and arguments.\n'));
            process.exit(-2);
        }
